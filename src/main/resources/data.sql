...
ALTER TABLE tb_users ADD CONSTRAINT uc_userentity_user_id UNIQUE (user_id, username, account_number);
CREATE INDEX idx_userentity_user_id ON tb_users(user_id, username, account_number);

CREATE TABLE tb_transaction_type (
  transaction_type_id DECIMAL NOT NULL,
...
ALTER TABLE tb_transaction_type ADD CONSTRAINT uc_transactiontype UNIQUE (transaction_type_id, transaction_code);
...
CREATE TABLE tb_transaction_history (
  transaction_history_id DECIMAL NOT NULL,
   activity_date TIMESTAMP NOT NULL,
   amount DECIMAL NOT NULL,
   user_id DECIMAL,
   transaction_type_id DECIMAL,
   CONSTRAINT pk_tb_transaction_history PRIMARY KEY (transaction_history_id)
);
ALTER TABLE tb_transaction_history ADD CONSTRAINT uc_tb_transaction_history_activity_date UNIQUE (activity_date);
...
VALUES
  (NEXTVAL('hibernate_sequence'), 'user 1', '$2a$10$uVIIvVx3B6BvViBT5kyJ1uH3d.iGyom52AqqMuNm4oTcWA1mcKbty', '1234567890'),
  (NEXTVAL('hibernate_sequence'), 'user 2', '$2a$10$83lqTeUR9gph734/u89OvudV77Sd9EukKDE5mL6FKL6MjF0blciaO', '0987654321');

INSERT INTO tb_transaction_type (transaction_type_id, transaction_code, transaction_name)
VALUES
  (NEXTVAL('hibernate_sequence'), '0001DEBIT', 'DEBIT ACCOUNT'),
  (NEXTVAL('hibernate_sequence'), '0002KREDIT', 'KREDIT ACCOUNT');